var OOCSI = (function () {
  ...
  function onMessage(evt) {
    if (evt.data !== "ping") {
      try {
        ...
        if (
          e.data.hasOwnProperty("_MESSAGE_ID") &&
          calls.hasOwnProperty(e.data["_MESSAGE_ID"])
        ) {
          ...
          handlers[e.recipient].forEach((fn) => {
            fn(e);
          });
        } else {
  ...
  function internalSend(client, data) {
    internalConnected() &&
      submit("sendjson " + client + " " + JSON.stringify(data));
  }
  ...
  return {
    ...
    variable: function (channel, name, throttle = 500) {
      ...
      function accessor(newValue, broadcast = true) {
        ...
        if (lastUpdate + throttle > Date.now()) {
          return;
        }
        lastUpdate = Date.now();
      ...
      this.subscribe(channel, function (e) {
        if (e.data.hasOwnProperty(name)) {
          accessor(e.data[name], false);
        }
      });
    ...
    heyOOCSI: function (name) {
      ...
      return {
        ...
        add_sensor_brick: function (
          ...
          logger(
            `Added ${sensor_name} to the components list for device ${device_name}.`
          );
        },
        add_number_brick: function (
          number_name,
          number_channel,
          number_min_max,
          number_unit,
          ...
          logger(
            `Added ${number_name} to the components list for device ${device_name}.`
          );
        },
        add_binary_sensor_brick: function (
          sensor_name,
          sensor_channel,
          sensor_type,
          sensor_default,
        ...
        add_switch_brick: function (
          ...
          logger(
            `Added ${switch_name} to the components list for device ${device_name}.`
          );
        },
        add_light_brick: function (
          light_name,
          light_channel,
          led_type,
          spectrum,
          ...
          logger(
            `Added ${light_name} to the components list for device ${device_name}.`
          );
        },
        submit: function () {
